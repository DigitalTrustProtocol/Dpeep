...
import { HandThumbDownIcon as HandThumbDownIconFull } from '@heroicons/react/24/solid';
import { useState } from 'preact/hooks';


const HandThumbDown = ({ onClick, size = 14, ...props }) => {
    ...
    return (
        <button {...props}  onClick={onClickHandler} className={className}  >
            <Show when={active}>
                <HandThumbDownIcon width={size} height={size}  />
            </Show>
            <Show when={!active}>
                <HandThumbDownIconFull width={size} height={size} />
            </Show>
        </button>
    );
}
