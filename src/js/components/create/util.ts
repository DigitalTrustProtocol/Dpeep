...
export async function sendNostr(msg: { text: string; replyingTo?: Event }) {
  const event = {
    kind: 1,
    content: msg.text,
  } as any;
  ...
  if (msg.replyingTo) {
    ...
    // });
    const replyingTo = msg.replyingTo;
    ...
    if (rootTag) {
      event.tags.unshift(['e', replyingTo.id, '', 'reply']);
      event.tags.unshift(['e', rootTag[1], '', 'root']);
    } else {
      event.tags.unshift(['e', replyingTo.id, '', 'root']);
    }
